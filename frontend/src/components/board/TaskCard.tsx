  ...
  return (
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 0.75 }}>
        ...
        <Tooltip arrow title={task.detail || task.title}>
          <Typography variant="body2" sx={{ flex: 1, fontWeight: 500, lineHeight: 1.3 }}>{task.title}</Typography>
        </Tooltip>
      </Box>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mt: 0.75, pl: 1.75 }}>
        ...
        <Box sx={{ flex: 1 }} />
        <IconButton size="small" aria-label="Complete" onClick={(e) => { e.stopPropagation(); onComplete(task.id) }} sx={{ p: 0.25 }}>
          <CheckCircle size={14} />
        </IconButton>
